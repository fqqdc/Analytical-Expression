...
namespace SyntaxAnalyzer
{
    public static class LRGrammarHelper
    {
        ...
        {
            var symbols = Action.Select(i => i.Key.t)
                .Distinct().ToArray();
            var states = Action.Keys.Select(key => key.state)
                .Distinct().OrderBy(s => s).ToArray();
        ...
        public static string ToFullString(this Dictionary<(int state, NonTerminal t), int> Goto)
        {
            var symbols = Goto.Select(i => i.Key.t)
                .Distinct().ToArray();
            var states = Goto.Keys.Select(key => key.state)
                .Distinct().OrderBy(s => s).ToArray();
