...
namespace SyntaxAnalyzer
{
    ...
    /// 带冲突的LR1文法
    /// </summary>
    public class CLR1Grammar : Grammar
    {
        protected CLR1Grammar(IEnumerable<Production> allProduction, NonTerminal startNonTerminal,
        ...
        public void ModifyAction(int state, Terminal t, List<ActionItem> list)
        {
            mapAction[(state,t)] = list.ToList();
        }

        public static CLR1Grammar Create(Grammar grammar)
        {
        ...
        public string ConflictMessage
        {
            get
            {
                StringBuilder sbConflictMsgMsg = new();
                foreach (var item in this.mapAction)
                {
                    if (item.Value.Count() > 1)
                        sbConflictMsgMsg.AppendLine($"ACTION {item.Key} 有多重入口：({string.Join(",", item.Value)})");
                }
                return sbConflictMsgMsg.ToString();
            }
        }
